...
$sql = "SELECT r.id, r.requisition_id, g.group_name, r.status 
        ...
        WHERE r.status = 'Patron Approved'";
        ...
        <tbody>
            <?php while ($row = $result->fetch_assoc()) { ?>
                <tr>
                    ...
                    <td>
                        <!-- Approve Requisition -->
                        <form action="approve_requisition_lcc.php" method="POST" style="display:inline;">
                            <input type="hidden" name="requisition_id" value="<?= $row['id'] ?>">
                            <button type="submit" class="btn btn-success">Approve</button>
                        </form>

                        <!-- Reject Requisition with comment -->
                        <form action="reject_requisition_lcc.php" method="POST" style="display:inline;">
                            <input type="hidden" name="requisition_id" value="<?= $row['id'] ?>">
